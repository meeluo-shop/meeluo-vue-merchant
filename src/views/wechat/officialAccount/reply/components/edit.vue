  ...
  <div class="wechat-container">
    ...
    <el-row :gutter="20" class="section-wrap">
      ...
      <el-card class="ml340">
        ...
        >
          <el-form-item v-if="type === '1'" label="关键字：" prop="keyword">
            <el-input
              v-model="replyForm.keyword"
              placeholder="请输入关键字"
            ></el-input>
            <!-- <span class="tip">关键字长度不得超过20</span> -->
          </el-form-item>
          ...
          <template v-if="replyForm.type === 50">
            ...
            <el-form-item label="选择图片：" prop="image_url">
              <vab-image-input v-model="image"></vab-image-input>
              <div slot="tip" class="el-upload__tip">
                文件最大2Mb，支持bmp/png/jpeg/jpg/gif格式
              </div>
            </el-form-item>
            <el-form-item label="图文链接" prop="link_url">
              <el-input
                v-model="replyForm.link_url"
                @input="textInput"
              ></el-input>
            </el-form-item>
            <el-form-item label="图文描述：" prop="introduction">
              <el-input
...
<script>
  ...
  import { getKeywordDetail, createKeyword, updateKeyword } from "@/api/wechat";
  import { uuid } from "@/utils/index";
  import SelectMedia from "./selectMedia";
  import { mount } from "sortablejs";

  export default {
    name: "WechatOfficialEditReply",
      ...
      return {
        ...
        replyFormRules: {
          keyword: [
            ...
            { max: 20, message: "关键字长度不得超过20", trigger: "blur" },
          ],
          type: [
    ...
    methods: {
      ...
      setTitle() {
        this.title = this.id ? "修改关键字回复" : "新增关键字回复";
      },
      async fetchData() {
        if (!this.id) {
          return;
        ...
        const { data } = await getKeywordDetail(this.id);
        this.detail = data;
        this.replyForm.keyword = data.keyword;
      ...
      async handleSave() {
        this.$refs["replyForm"].validate(async (valid) => {
          if (valid) {
            ...
            try {
              if (this.id) {
                await updateKeyword(this.id, this.replyForm);
                this.$baseMessage("修改成功", "success");
              } else {
                await createKeyword(this.replyForm);
                this.$baseMessage("添加成功", "success");
              }
              await this.routerBack();
              loading.close();
            } catch (err) {
              loading.close();
            }
